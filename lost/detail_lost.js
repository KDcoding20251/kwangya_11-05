...
async function loadLostDetail() {
    const lostId = getLostIdFromUrl();
    if (!lostId) return;

    const lostRef = doc(db, "losts", lostId);
    const lostDoc = await getDoc(lostRef);

    if (lostDoc.exists()) {
        const lost = lostDoc.data();
        const authorUid = lost.authorUid; 
        
        // 🚨 닉네임 조회 로직 제거: authorNickname = await getNicknameByUid(authorUid);
        
        document.getElementById('lostTitle').textContent = lost.title;
        document.getElementById('lostContent').textContent = lost.content;
        
        // ⭐ 수정: 닉네임 대신 UID나 '작성자' 텍스트 표시
        const authorElement = document.getElementById('lostAuthor'); 
        if (authorElement) {
        ...
        document.getElementById('lostTitle').textContent = "게시물을 찾을 수 없습니다.";
    }
...
async function submitComment(lostId) {
    ...
    await addDoc(collection(db, "losts", lostId, "comments" ), {
...
async function loadComments(lostId) {
    const commentsList = document.getElementById('commentsList');
    commentsList.innerHTML = '';
    
    const commentsQuery = query(
        collection(db, "losts", lostId, "comments"), 
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    onAuthStateChanged(auth, (user) => {
        ...
        loadLostDetail(lostId); 
    });
    
    document.getElementById('submitComment').addEventListener('click', () => {
        submitComment(lostId);
    });
});