...
document.addEventListener('DOMContentLoaded', () => {
    ...
    onAuthStateChanged(auth, async (user) => {
        ...
        document.getElementById('submitLost').addEventListener('click', saveLost);
...
async function saveLost() {
    // 1. 제목과 내용 필드 가져오기
    const lostTitleElement = document.getElementById('lostTitle');
    const lostContentElement = document.getElementById('lostContent');
    
    const title = lostTitleElement.value.trim();
    const content = lostContentElement.value.trim();
    ...
    try {
        ...
        const docRef = await addDoc(collection(db, LOSTS_COLLECTION), {
        ...
        // 4. 입력 필드 초기화
        lostTitleElement.value = ''; 
        lostContentElement.value = ''; 
...
async function loadLosts() {
    const lostsListElement = document.getElementById('lostsList');
    lostsListElement.innerHTML = '<p>게시물을 불러오는 중...</p>'; // 로딩 표시

    try {
        // 'anonymousPosts' 컬렉션에서 'createdAt' 기준으로 내림차순 정렬하여 쿼리
        const q = query(collection(db, LOSTS_COLLECTION), orderBy('createdAt', 'desc'));
        
        // 쿼리 실행
        const querySnapshot = await getDocs(q);
        
        // 게시물 목록 초기화
        lostsListElement.innerHTML = ''; 

        if (querySnapshot.empty) {
            lostsListElement.innerHTML = '<p>아직 게시물이 없습니다.</p>';
            return;
        }

        // 각 문서(게시물)를 순회하며 HTML 요소 생성
        querySnapshot.forEach((doc) => {
            const lostId = doc.id; // Firestore 문서 ID를 게시물 ID로 사용
            const lost = doc.data(); // 문서 데이터 가져오기
            // 타임스탬프를 읽기 쉬운 형태로 변환
            const date = new Date(lost.createdAt);
            const formattedDate = date.toLocaleString();

            const lostDiv = document.createElement('div');
            lostDiv.className = 'lost-item';
            
            lostDiv.innerHTML = `
                <h3><a href="../lost_detail.html?id=${lostId}">${lost.title}</a></h3> 
                <p>작성자: ${lost.author} | ${formattedDate}</p>
                <hr>
            `;
            
            lostsListElement.appendChild(lostDiv);
        });

    } catch (e) {
        console.error("게시물 로드 중 오류 발생: ", e);
        lostsListElement.innerHTML = '<p style="color: red;">게시물을 불러오는 데 실패했습니다.</p>';
    }
}

// 6. 페이지 로드 시 게시물 불러오기
document.addEventListener('DOMContentLoaded', loadLosts);